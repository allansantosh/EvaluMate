 ...
 */
 ...
 var websocketclient = {
    ...
    'onConnect': function () {
        ...
        websocketclient.subscribe('cloud_project/'+ poll_id + '/connection', 1, 999999);
        websocketclient.subscribe('cloud_project/'+ poll_id + '/current_question', 1, 999999);
        websocketclient.subscribe('cloud_project/'+ poll_id + '/get_att', 1, 999999);
       ...
       // websocketclient.subscribe('cloud_project/'+ poll_id + '/event_capture_progress', 1 , 999999);
         websocketclient.publish('cloud_project/'+ poll_id + '/check_connection', 'check_connection_now', 1, false);
         websocketclient.publish('cloud_project/'+ poll_id + '/att', 'present', 1, false);
         websocketclient.publish('cloud_project/'+ poll_id + '/get_current_question', 'get_current_question', 1, false);
...
//        console.log("onMessageArrived:" + message.payloadString + " qos: " + message.qos);
    ...
    'render': {
        ...
        'message': function (message) {
            ...
            var res = the_message.split(" ");
            var poll_id = $('#poll_id').val();

            if(the_message == "get_att_now"){
                websocketclient.publish('cloud_project/'+ poll_id + '/att', 'present', 1, false);
            }
            
            if(the_message == "Live"){
                document.getElementById("connection_status").innerHTML = "<span style='color: #ccc;'>" + the_message + "</span>";
              /// document.getElementById("led_status").innerHTML = "<span style='color: #15bf25;'>" + the_message + "</span>";
               // document.getElementById("led_status").disabled = false;
            ...
            if(the_message == "Offline"){
                document.getElementById("connection_status").innerHTML = "<span style='color: #ccc;'>Waiting...</span>";
                document.getElementById("current_question").innerHTML = "Please wait for the instructor to connect.";
                //document.getElementById("bulb_image").src = "../bulb_off.png";
                //document.getElementById("led_status").innerHTML = "LED Status: Unknown";
                //document.getElementById("record_event_1").innerHTML = "Accident Capture Not Avaliable Now";
                //document.getElementById("record_event_2").innerHTML = "Crowd Control Capture Not Avaliable Now";
                //document.getElementById("led_status").disabled = true;
                //document.getElementById("record_event_1").disabled = true;
                //document.getElementById("record_event_2").disabled = true;
                setTimeout(() => window.location.reload(), 2000);
            }

            if(res[0] == "current_question"){

                if (res[1] == "not_started") {
                document.getElementById("current_question").innerHTML = "Instructor connected. Please wait for the assessment to begin.";
            }

             else if (res[1] == "finish"){
                document.getElementById("submit_poll").style.display = "block";
             }
                ...
                for (i=1; i <= number_of_questions; i++) {
                    ...
                    document.getElementById("poll_loading_image").style.display = "none";
                    var question = "question_" + res[1];
