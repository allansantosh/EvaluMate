...
var websocketclient = {
    ...
    'onConnect': function () {
        ...
        websocketclient.subscribe('cloud_project/'+ poll_id + '/check_connection', 1, 999999);
        websocketclient.subscribe('cloud_project/'+ poll_id + '/get_current_question', 1, 999999);
        websocketclient.subscribe('cloud_project/'+ poll_id + '/att', 1, 999999);
       // websocketclient.subscribe('cloud_project/'+ poll_id + '/LED', 1 ,999999);
       // websocketclient.subscribe('cloud_project/'+ poll_id + '/event_capture_progress', 1 , 999999);
       // websocketclient.publish('cloud_project/'+ poll_id + '/check_connection', 'check connection now', 1, false);
          websocketclient.publish('cloud_project/'+ poll_id + '/get_att', 'get_att_now' , 1, false);
        //websocketclient.publish('cloud_project/'+ poll_id + '/check_led', 'check led now' , 1, false);
...
//        console.log("onMessageArrived:" + message.payloadString + " qos: " + message.qos);
    ...
    'render': {
        ...
        'message': function (message) {
            ...
            var the_message = Encoder.htmlEncode(message.payload);
            var poll_id = $('#poll_id').val();

            if(the_message == "present"){

                var current_number = parseInt(document.getElementById("students_connected_inner").innerHTML);
                current_number = current_number + 1;
                document.getElementById("students_connected_inner").innerHTML = current_number;
            }

            if(the_message == "absent"){
            ...
            if(the_message == "check_connection_now"){
               // document.getElementById("led_status").innerHTML = "Turn LED OFF";
               // document.getElementById("bulb_image").src = "../bulb_on.png";
             //  console.log("dfb");
                websocketclient.publish('cloud_project/'+ poll_id + '/connection', 'Live', 1, false);
               // console.log("Sdvsdv");
            }

            if(the_message == "get_current_question"){
                // document.getElementById("led_status").innerHTML = "Turn LED OFF";
                // document.getElementById("bulb_image").src = "../bulb_on.png";
                console.log("dfb");
                var current_question = $('#current_question').val();
                websocketclient.publish('cloud_project/'+ poll_id + '/current_question', 'current_question ' + current_question , 1, false);
                 console.log(current_question);
             }

            if(the_message == "OFF"){
                document.getElementById("led_status").innerHTML = "Turn LED ON";
                //document.getElementById("bulb_image").src = "../bulb_off.png";
            }

            if(the_message == "capture_1_done"){
                document.getElementById("record_event_1").disabled = false;
                document.getElementById("record_event_1").innerHTML = "Capture Accident Now";
                alert("Event Capture 1 Successful!")
            }

            if(the_message == "capture_2_done"){
                document.getElementById("record_event_2").disabled = false;
                document.getElementById("record_event_2").innerHTML = "Capture Event Now";
                alert("Event Capture 2 Successful!")
            }
